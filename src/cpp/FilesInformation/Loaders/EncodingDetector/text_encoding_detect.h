...
namespace Common
{
	...
	public:
		...
		static Encoding CheckBOM(const char *pBuffer, size_t size);		// Just check if there is a BOM and return 
		Encoding DetectEncoding(const char *pBuffer, size_t size) const;	// Check BOM and also guess if there is no BOM
	...
	private:
		...
		static const char* utf16_bom_le_;
		static const char* utf16_bom_be_;
		static const char* utf8_bom_;

		bool	null_suggests_binary_;
		int		utf16_expected_null_percent_;
		int		utf16_unexpected_null_percent_;

		Encoding CheckUTF8(const char *pBuffer, size_t size) const;				// Check for valid UTF8 with no BOM
		static Encoding CheckUTF16NewlineChars(const char *pBuffer, size_t size);	// Check for valid UTF16 with no BOM via control chars
		Encoding CheckUTF16ASCII(const char *pBuffer, size_t size) const;		// Check for valid UTF16 with no BOM via null distribution
		static bool DoesContainNulls(const char *pBuffer, size_t size);			// Check for nulls
	};
